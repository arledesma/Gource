...
public:
    ...
    RCommitFile(const std::string& filename, const std::string& action, vec4 colour);
};

class RCommit {
    vec4 fileColour(const std::string& filename);
public:
    time_t timestamp;
    std::string username;
    ...
    void addFile(const std::string& filename, const std::string& action, const vec4& colour);
